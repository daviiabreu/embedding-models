...
class InteliHTMLProcessor:
    ...
    NOISE_PATTERNS = {
        'ids': [
            ...
            'footer', 'comments', 'social', 'share', 'related',
            'cookie', 'banner', 'ads', 'advertisement'
        ],
        'classes': [
            'nav', 'menu', 'header', 'footer', 'sidebar', 'share',
            'social', 'comment', 'related', 'cookie', 'banner',
            'breadcrumb', 'pagination', 'author-box', 'meta',
            'tags', 'categories', 'newsletter', 'subscribe'
        ],
        'tags': [
    ...
    def is_noise_element(self, element: Tag) -> bool:
        ...
        try:
            elem_classes = ' '.join(element.get('class', [])).lower() if element.get('class') else ''
        except (AttributeError, TypeError):
            elem_classes = ''
        
        if elem_classes and any(noise in elem_classes for noise in self.NOISE_PATTERNS['classes']):
            return True
    ...
    def extract_hierarchy(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        print(f"✅ Processando <body> completo")
            ...
            try:
                ...
                if tag_name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
                    ...
                    elements.append({
                        ...
                        'parent_sections': [h['title'] for h in hierarchy_stack[:-1]]
                    })
    ...
    def group_by_semantic_context(self, elements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        while i < len(elements):
            ...
            if current_type == 'ListItem':
                ...
                while j < len(elements):
                    ...
                    if (next_elem.get('type') == 'ListItem' and
                        next_elem.get('context_section') == current_section):
            ...
            # ========================================
            elif (current_type == 'NarrativeText' and 
                  i + 1 < len(elements)):
                ...
                if (next_section == current_section and
                    next_type == 'NarrativeText' and
                    len(current_text) < 100 and  # Nome curto
                    len(next_text) > 50):  # Descrição mais longa
        ...
        if reduction > 0:
            print(f"   ✅ Consolidação aplicada:")
            print(f"      • Elementos antes: {len(elements)}")
            print(f"      • Elementos depois: {len(grouped)}")
            print(f"      • Redução: {reduction} ({reduction/len(elements)*100:.1f}%)")
    ...
    def batch_process_directory(self, dir_path: Path) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        for html_file in html_files:
            ...
            if elements:
                ...
                print(f"   ⚠️ Nenhum elemento extraído")
    ...
    for idx, element in enumerate(elements):
        ...
        pipeline_elements.append({
            ...
            'metadata': {
                ...
                'context_header': element.get('parent_sections', [])[-1] if element.get('parent_sections') else '',
