...
def display_processing_results(chunks, original_filename):
    ...
    with col4:
        sections = set(
            chunk["metadata"].get("section_context", "N/A") for chunk in chunks
        )
    ...
    with st.expander("👀 Preview dos Chunks"):
        num_preview = st.slider(
            "Número de chunks para preview", 1, min(10, len(chunks)), 3
        )
        ...
        for i in range(num_preview):
            ...
            if len(content) > 300:
                ...
                if st.button("Ver completo", key=f"expand_{i}"):
                    st.write(content)
            else:
                st.write(content)
