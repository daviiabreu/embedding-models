...
def check_content_safety(user_input: str, tool_context: ToolContext) -> dict:
    ...
    result = {"is_safe": len(found_unsafe) == 0, "input": user_input, "reason": ""}

    if not result["is_safe"]:
        result["reason"] = (
            f"Contains potentially harmful content: {', '.join(found_unsafe)}"
        )
        ...
        tool_context.state["safety_violations"].append(
            {"input": user_input, "violations": found_unsafe}
        )
