...
def add_dog_personality(text: str, emotion: str, tool_context: ToolContext) -> dict:
    ...
    expressions = {
        ...
        "excited": {
            ...
            "actions": ["*pula de alegria*", "*gira em círculos*", "*balança o rabo rapidamente*"],
            "intensifiers": ["extremamente", "incrivelmente", "fantasticamente"]
        },
        "calm": {
            "barks": ["[latido suave]", "[latido gentil]"],
            "actions": ["*inclina a cabeça*", "*senta ao seu lado*", "*olha atentamente*"],
        ...
        "empathetic": {
            ...
            "actions": ["*se aproxima gentilmente*", "*coloca a pata no seu braço*", "*olha nos seus olhos*"],
...
def detect_visitor_emotion(visitor_input: str, tool_context: ToolContext) -> dict:
    ...
    emotion_patterns = {
        "excited": {
            "keywords": ["incrível", "demais", "adorei", "amei", "uau", "wow", "legal", "massa", "!"],
            "weight": 1.0
        },
        "happy": {
            "keywords": ["feliz", "bom", "ótimo", "sim", "obrigado", "valeu", "show", "rs", "haha", "kkk"],
            "weight": 0.9
        },
        "curious": {
            "keywords": ["como", "por que", "quando", "onde", "qual", "?", "quero saber", "me conte"],
            "weight": 0.8
        },
        "confused": {
        ...
        "anxious": {
            "keywords": ["preocupado", "ansioso", "medo", "difícil", "complicado", "nervoso"],
            "weight": 0.6
        },
        "bored": {
            "keywords": ["ok", "tá", "tanto faz", "sei lá"],
            "weight": 0.5
        },
        "neutral": {
            "keywords": [".", "próximo", "continua", "vamos"],
            "weight": 0.3
        }
    }
    ...
    if len(tool_context.state['visitor_emotions']) > 10:
        tool_context.state['visitor_emotions'] = tool_context.state['visitor_emotions'][-10:]
