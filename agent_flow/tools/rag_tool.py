"""ZenML steps and helpers for the Inteli RAG inference service."""
...
DEFAULT_ADJACENT_LIMIT = int(os.getenv("RAG_ADJACENT_LIMIT", "1"))
...
def _convert_to_qdrant_id(value: Any) -> Any:
    ...
    if isinstance(value, (int,)):  # pragma: no cover - simple helper
        return value
...
def _extract_adjacency_candidates(payload: Dict[str, Any], metadata: Dict[str, Any]) -> Any:
...
def _normalize_adjacency_ids(adjacency_raw: Any, limit: int) -> Tuple[List[str], List[Any]]:
...
def _retrieve_adjacency_payloads(client: QdrantClient, adjacency_ids: Sequence[Any]) -> Dict[str, Dict[str, Any]]:
...
def query_embedding_step(query: str) -> List[float]:
    """ZenML step wrapper for `compute_query_embedding`."""
    return compute_query_embedding(query)
...
def retrieval_from_qdrant_step(
    ...
    """ZenML step wrapper for `retrieve_from_qdrant`."""
...
@step(enable_cache=False)
def build_graph_rag_payload_step(
    query: str,
    query_embedding: List[float],
    retrieved_nodes: List[Dict[str, Any]],
) -> Dict[str, Any]:
    """ZenML step wrapper for `build_graph_rag_payload`."""
    return build_graph_rag_payload(
        query=query,
        query_embedding=query_embedding,
...
def rag_inference_pipeline(
    ...
    retrieval = retrieval_from_qdrant_step(
        query_embedding=query_embedding,
        top_k=top_k,
        adjacency_limit=adjacency_limit,
    )
    payload = build_graph_rag_payload_step(
        query=query,
        query_embedding=query_embedding,
...
def run_graph_rag_tool(
    query: str,
    top_k: Optional[int] = None,
    adjacency_limit: Optional[int] = None,
) -> Dict[str, Any]:
    """Convenience helper to execute the ZenML graph RAG pipeline programmatically."""
    resolved_top_k = top_k or DEFAULT_TOP_K
    resolved_limit = adjacency_limit or DEFAULT_ADJACENT_LIMIT
    query_embedding = compute_query_embedding(query=query)
    retrieval = retrieve_from_qdrant(
        query_embedding=query_embedding,
        top_k=resolved_top_k,
        adjacency_limit=resolved_limit,
    )
    return build_graph_rag_payload(
        query=query,
        query_embedding=query_embedding,
