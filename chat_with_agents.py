...
def print_header():
    print("\n" + "█" * 70)
    print(f"{BOLD}🤖 INTERFACE DE CHAT - SISTEMA MULTI-AGENTES INTELI (DEBUG MODE){RESET}")
    print("█" * 70)
...
def print_rag_debug(orchestrator, user_input):
    ...
    try:
        ...
        if not context_raw:
            print(f"{YELLOW}⚠️  RAG Debug: Nenhum contexto relevante encontrado no Qdrant.{RESET}")
            return
